...
```{r activity}
projects$last_update <- as_date(projects$last_update)
# weekly active projects
last_week <- today() - weeks(1)
weekly_active <- projects$last_update >= last_week
weekly_active_proj <- sum(weekly_active)
weekly_active_user <- length(unique(projects$user[weekly_active]))

# monthly active projects
last_month <- today() - months(1)
monthly_active <- projects$last_update >= last_month
monthly_active_proj <- sum(monthly_active)
